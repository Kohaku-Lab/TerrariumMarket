<template>
  <div class="container-page">
    ...
    <section v-if="!auth.signedIn" class="card p-12 flex flex-col items-center text-center gap-3">
      ...
      <p class="text-sm text-warm-500 dark:text-warm-400 max-w-md">Sign in with GitHub to submit packages, post in the forum, and track your contributions.</p>
    ...
    <template v-else>
      <section class="card p-6 mb-6 flex items-center gap-4">
        <img v-if="auth.user?.avatar_url" :src="auth.user.avatar_url" alt="" class="w-16 h-16 rounded-full" />
        <div class="flex-1">
          ...
          <a :href="auth.user?.html_url" target="_blank" rel="noopener noreferrer" class="text-[12px] text-iolite dark:text-iolite-light hover:underline inline-flex items-center gap-1 mt-1">
      ...
      <section class="card p-6">
        ...
        <p class="text-[13px] text-warm-600 dark:text-warm-400 mb-4 leading-relaxed">
          ...
          scope. That's enough to fork the registry, push a branch, open a PR, and post to public Discussions. No access to private repos, no SSH keys, no email changes.
        </p>
        <p class="text-[13px] text-warm-600 dark:text-warm-400 mb-4 leading-relaxed">The token is stored locally in your browser — sign out removes it. You can also revoke it any time from GitHub's settings.</p>
        <a href="https://github.com/settings/applications" target="_blank" rel="noopener noreferrer" class="btn-secondary inline-flex items-center">
