<template>
  <div class="container-page max-w-3xl">
    <h1 class="text-2xl font-semibold text-warm-800 dark:text-warm-200 mb-4">
      About {{ SITE_NAME }}
    </h1>
    ...
    <div class="card p-6 mb-6">
      <p class="text-[14px] text-warm-700 dark:text-warm-300 leading-relaxed mb-4">
        ...
        <a
          href="https://github.com/Kohaku-Blueleaf/KohakuTerrarium"
          target="_blank"
      ...
      <p class="text-[14px] text-warm-700 dark:text-warm-300 leading-relaxed mb-4">
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          class="text-iolite dark:text-iolite-light hover:underline font-mono"
          >{{ REGISTRY_OWNER }}/{{ REGISTRY_REPO }}</a
        >. No database, no backend service. Submissions are PRs; the forum is GitHub Discussions on
        the same repo; auth is GitHub Device Flow.
      </p>

      <p class="text-[14px] text-warm-700 dark:text-warm-300 leading-relaxed">
        This site is a static Vue 3 app deployed on Cloudflare Pages, reading the registry over
        plain HTTPS. You can fork the data repo and run a competing registry — point the
        KohakuTerrarium client at your fork's
    ...
    <section class="card p-6 mb-6">
      ...
      <ul class="space-y-3 text-[14px] text-warm-700 dark:text-warm-300">
        <li class="flex items-start gap-3">
          <span
            class="i-carbon-add-large text-iolite dark:text-iolite-light text-[18px] shrink-0 mt-0.5"
          />
          <div>
            <router-link
              :to="{ name: 'submit' }"
              class="font-medium text-iolite dark:text-iolite-light hover:underline"
              >Submit a package</router-link
            >
        ...
        <li class="flex items-start gap-3">
          <span
            class="i-carbon-chat text-iolite dark:text-iolite-light text-[18px] shrink-0 mt-0.5"
          />
          <div>
            <router-link
              :to="{ name: 'forum' }"
              class="font-medium text-iolite dark:text-iolite-light hover:underline"
              >Join the forum</router-link
            >
        ...
        <li class="flex items-start gap-3">
          <span
            class="i-carbon-logo-github text-iolite dark:text-iolite-light text-[18px] shrink-0 mt-0.5"
          />
          <div>
            <a
              :href="`https://github.com/${REGISTRY_OWNER}/${REGISTRY_REPO}/blob/main/CONTRIBUTING.md`"
              target="_blank"
              rel="noopener noreferrer"
              class="font-medium text-iolite dark:text-iolite-light hover:underline"
    ...
    <section class="card p-6">
      ...
      <a
        :href="registry.metadata?.source || ''"
        target="_blank"
        rel="noopener noreferrer"
        class="text-[12px] text-iolite dark:text-iolite-light hover:underline inline-flex items-center gap-1 mt-4"
      >
