<template>
  <div class="container-page">
    <header class="mb-6">
      ...
      <p class="text-sm text-warm-500 dark:text-warm-400 max-w-2xl">
        Fill the form below and we'll open a pre-filled GitHub editor in a new tab. GitHub handles
        the rest — login, forking, branching, opening the PR. A maintainer reviews + merges; the
        on-merge workflow rebuilds the index and Cloudflare Pages auto-deploys within a minute.
      </p>
    </header>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <section class="card p-6 lg:col-span-2 flex flex-col gap-4">
        <FieldRow
          label="Package name"
          hint="3–40 chars; lowercase letters, digits, `-`, `_`. Becomes the install id (kt install @<name>)."
        >
          <input
            v-model="form.name"
            type="text"
            class="input-field font-mono"
            placeholder="my-creature-pack"
        ...
        <FieldRow
          label="GitHub repo"
          hint="Public HTTPS URL to a repo with a kohaku.yaml at its root."
        >
          <input
            v-model="form.repo"
            type="url"
            class="input-field font-mono"
            placeholder="https://github.com/your-name/my-creature-pack"
            :class="{ '!border-coral': errors.repo }"
          />
        </FieldRow>

        <FieldRow
          label="One-line description"
          hint="≤ 200 chars; plain text; shown on the browse card."
        >
          <input
            ...
            maxlength="200"
            class="input-field"
            placeholder="A focused creature for code-review sessions"
          />
        </FieldRow>

        <FieldRow
          label="Tags"
          hint="Comma-separated. Must include at least one of: creatures, terrariums, plugins, tools."
        >
          <input
            v-model="tagsString"
            type="text"
            class="input-field"
            placeholder="creatures, examples"
            :class="{ '!border-coral': errors.tags }"
          />
        </FieldRow>
        ...
        <FieldRow
          label="Framework constraint"
          hint="PEP 440 / semver-style. Defaults to >=1.5.0,<2.0.0."
        >
          <input
            v-model="form.framework"
            type="text"
            class="input-field font-mono"
            placeholder=">=1.5.0,<2.0.0"
          />
        </FieldRow>

        <div class="border-t border-warm-200 dark:border-warm-700 pt-4">
          <h2 class="text-[12px] uppercase tracking-wider text-warm-500 dark:text-warm-400 mb-3">
            First version
          </h2>
      ...
      <aside class="flex flex-col gap-4">
        <section class="card p-4">
          <h3 class="text-[12px] uppercase tracking-wider text-warm-500 dark:text-warm-400 mb-2">
            entry.yaml preview
          </h3>
          <pre
            class="font-mono text-[11px] leading-snug text-warm-700 dark:text-warm-300 whitespace-pre-wrap break-all overflow-auto max-h-[400px]"
            >{{ yamlPreview || "// fill the form to preview…" }}</pre
          >
        </section>
        ...
        <section class="card p-4 text-[12px] text-warm-600 dark:text-warm-400 leading-relaxed">
          ...
          <ul class="space-y-2 list-disc pl-4">
            <li>
              Make sure your repo's <code>kohaku.yaml</code> declares the same <code>name</code>.
            </li>
            <li>Push and tag a git tag matching <em>Version tag</em> above.</li>
            <li>
              The PR CI validates the schema, SPDX licence, and that the tag exists in your source
              repo. Errors will show up on the PR.
            </li>
...
const FieldRow = (props, { slots }) =>
  h("label", { class: "flex flex-col gap-1.5" }, [
    h("div", { class: "text-[12px] font-medium text-warm-600 dark:text-warm-400" }, props.label),
    slots.default?.(),
    props.hint
      ? h("div", { class: "text-[11px] text-warm-400 dark:text-warm-500 leading-snug" }, props.hint)
      : null,
  ])
